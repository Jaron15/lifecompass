...
function Notes({ hobby }) {
    ...
    return (
        <div className="rounded h-full flex flex-col">
            ...
            <div className="flex justify-between items-center bg-transparent mx-4">
                <h2 className="text-2xl font-bold">Notes</h2>
                <button onClick={handleCreateNew} className="text-blue-500 text-3xl rounded">+</button>
            </div>
                    ...
                    {allNotes.map(note => (
                        <li key={note.id} onClick={() => handleNoteClick(note)} className="flex justify-between items-start bg-white p-4 rounded mb-2 shadow cursor-pointer dark:bg-boxdark text-black dark:text-current">
                            <div className='felx flex-col w-full' >
                                <h3 className="text-2xl font-bold truncate w-full">{note.header}</h3>
            ...
            <div className="w-2/3 bg-white p-4 rounded shadow h-full dark:bg-boxdark text-black dark:text-current">
                {creating ? (
                    ...
                    <button className='bg-blue-500 h-15 w-35 rounded p-2' onClick={handleSaveNewNote}>
                      Save
                    </button>
                  </div>
                  </div>
                    ...
                    <div className="h-full flex flex-col justify-center items-center  text-black dark:text-current">
                        ...
                        <p className=" mt-4">...or create a new one using the + button.</p>
                    </div>
                )}
            </div>
